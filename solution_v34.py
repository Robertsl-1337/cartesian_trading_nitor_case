...
print(f"Train: {train.delivery_start.min().date()} → {train.delivery_start.max().date()}")
print(f"Test:  {test.delivery_start.min().date()}  → {test.delivery_start.max().date()}")
...
print(f"\nRemoved {n_removed} outliers — target: [{train.target.min():.1f}, {train.target.max():.1f}]")

# ─────────────────────────────────────────────────────────────────────────────
# 3. DAY-LEVEL FEATURES
# ─────────────────────────────────────────────────────────────────────────────
def add_day_features(df):
    d = df.copy(); d['_date'] = d['delivery_start'].dt.date
...
train = add_day_features(train); test = add_day_features(test)
print(f"Day features added.")
...
G_MEAN = recent_te['target'].mean(); G_STD = recent_te['target'].std()
...
train = add_te(train); test = add_te(test)
print(f"Target encoding done. G_MEAN={G_MEAN:.2f}")

# ─────────────────────────────────────────────────────────────────────────────
# 5. CORE FEATURES (identical to V28/V33)
# ─────────────────────────────────────────────────────────────────────────────
WEATHER = ['global_horizontal_irradiance','diffuse_horizontal_irradiance',
...
def make_features(df, ref):
    df = df.copy()
    for col in WEATHER:
        if col in df.columns: df[col] = df[col].fillna(ref[col].median())
    ...
    lfc = df['load_forecast']; rfc = sf + wfc; bal = lfc - rfc
    ...
    df['cloud_w']    = 0.5*cclo + 0.3*df['cloud_cover_mid'].fillna(0) + 0.2*df['cloud_cover_high'].fillna(0)
...
print(f"Core features: {train_fe.shape}")

# ─────────────────────────────────────────────────────────────────────────────
# 6. CROSS-MARKET + INTRADAY FEATURES
# ─────────────────────────────────────────────────────────────────────────────
def add_cross_market_features(df):
    cross = df.groupby('delivery_start').agg(
        xmkt_load_mean=('load_forecast','mean'), xmkt_solar_mean=('solar_forecast','mean'),
        xmkt_wind_mean=('wind_forecast','mean'), xmkt_balance_mean=('balance','mean'),
...
def add_intraday_features(df):
    df = df.copy(); df['_date'] = df['delivery_start'].dt.date
    df_s = df.sort_values(['market','delivery_start'])
    for col in ['load_forecast','solar_forecast','wind_forecast','balance']:
        m = df_s.groupby(['market','_date'])[col].transform(lambda x: x.rolling(3, min_periods=1).mean())
        s = df_s.groupby(['market','_date'])[col].transform(lambda x: x.rolling(3, min_periods=1).std().fillna(0))
        df.loc[df_s.index, f'{col}_3h_mean'] = m.values
        df.loc[df_s.index, f'{col}_3h_std']  = s.values
...
train_fe = add_cross_market_features(train_fe); test_fe = add_cross_market_features(test_fe)
train_fe = add_intraday_features(train_fe);    test_fe = add_intraday_features(test_fe)

# ─────────────────────────────────────────────────────────────────────────────
# 7. 1-YEAR LAGS (identical to V28/V33)
# ─────────────────────────────────────────────────────────────────────────────
print("\nBuilding lag features...")
lag_src = train[['market', 'delivery_start', 'target']].copy()

lag_364 = lag_src.copy(); lag_364['delivery_start'] += pd.Timedelta(days=364)
lag_364 = lag_364.rename(columns={'target':'lag1y_364'}).drop_duplicates(['market','delivery_start'])
lag_365 = lag_src.copy(); lag_365['delivery_start'] += pd.Timedelta(days=365)
...
for offset in OFFSETS:
    lo = lag_src.copy(); lo['delivery_start'] += pd.Timedelta(days=offset)
...
lag_m_agg = (train.assign(_yr=train['delivery_start'].dt.year)
             .groupby(['_yr','month','market','hour_of_week'])['target']
             .agg(lag1y_mhow_mean='mean', lag1y_mhow_std='std').reset_index())
lag_m_agg['_yr'] = lag_m_agg['_yr'] + 1

def add_monthly_lags(df, te_col='te_hdow_mean'):
    df = df.copy(); df['_yr'] = df['delivery_start'].dt.year
...
def add_lag_interactions(df):
    df = df.copy()
    lv=df['lag1y_vs_te']; la=df['lag1y_avg']; lm=df['lag1y_mhow_vs_te']
...
print(f"Lag features done. Train: {train_fe.shape}")

# ─────────────────────────────────────────────────────────────────────────────
# 8. FEATURE MATRIX + HOLDOUT SPLIT
# ─────────────────────────────────────────────────────────────────────────────
train_fe = pd.get_dummies(train_fe, columns=['market'], prefix='mkt')
test_fe  = pd.get_dummies(test_fe,  columns=['market'], prefix='mkt')

DROP = ['id','target','delivery_start','delivery_end','hour','dow','month','doy','hour_of_week','week']
feature_cols = [c for c in train_fe.columns
                if c not in DROP and c in test_fe.columns and train_fe[c].dtype != 'object']
...
print(f"\nFeatures: {len(feature_cols)}  ({len(pm_cols)} per-market)")
print(f"Train: {mask_train.sum():,}  Hold-2023: {mask_holdout1.sum():,}  Hold-2024: {mask_holdout2.sum():,}")

# ─────────────────────────────────────────────────────────────────────────────
# 9. REGIME CLUSTERING
# ─────────────────────────────────────────────────────────────────────────────
DAY_RF = [c for c in ['day_balance','day_renew_pct','day_temp_avg','day_hdd','day_cdd',
                       'day_solar_sum','day_wind_sum','day_load_sum'] if c in feature_cols]
fidx_r = [feature_cols.index(f) for f in DAY_RF]
scaler_r = StandardScaler()
X_reg_tr = scaler_r.fit_transform(X[tr_idx][:, fidx_r])
km = KMeans(n_clusters=8, random_state=42, n_init=10); km.fit(X_reg_tr)
...
def train_lgb(cfg, Xtr, ytr, sw_tr, Xes, yes):
    ...
    m.fit(Xtr, ytr, sample_weight=sw_tr, eval_set=[(Xes, yes)],
...
LGB_DEEP = dict(n_estimators=4000, learning_rate=0.02,  num_leaves=127,
                ...
                bagging_freq=5, reg_alpha=0.05, reg_lambda=0.1, verbose=-1, n_jobs=-1, random_state=42)
LGB_REG  = dict(n_estimators=3000, learning_rate=0.03,  num_leaves=63,
                ...
                bagging_freq=5, reg_alpha=0.4, reg_lambda=0.8, verbose=-1, n_jobs=-1, random_state=42)
...
# Base XGB configs — seed will be set per model
...
DART_BASE = dict(boosting_type='dart', n_estimators=1500, learning_rate=0.025,
                 ...
                 bagging_fraction=0.8, bagging_freq=5, drop_rate=0.1, skip_drop=0.5,
                 max_drop=50, reg_alpha=0.05, reg_lambda=0.1, verbose=-1, n_jobs=-1)

LGB_GLOBAL_D127 = dict(n_estimators=4000, learning_rate=0.02, num_leaves=127,
                        min_child_samples=15, feature_fraction=0.8, bagging_fraction=0.8,
                        bagging_freq=5, reg_alpha=0.05, reg_lambda=0.1, verbose=-1, n_jobs=-1, random_state=123)
LGB_GLOBAL_D255 = dict(n_estimators=4000, learning_rate=0.02, num_leaves=255,
                        ...
                        bagging_freq=5, reg_alpha=0.1, reg_lambda=0.3, verbose=-1, n_jobs=-1, random_state=123)

# Seeds for the zoo
D7_SEEDS   = [42, 123, 456, 789, 999, 314, 2024]   # 7 models
D8_SEEDS   = [42, 123, 456, 789, 999]               # 5 models
DART_SEEDS = [42, 123, 456, 789]                    # 4 models
# Total global: 7 + 5 + 2 + 4 = 18
# Total with pm: 18 + 2 = 20
...
for mkt in MARKETS:
    ...
    Xm_tr = X[tr_m][:,pm_idx]; Xm_te = X_test[te_m][:,pm_idx]
    ...
    test_lgb_deep[te_m] = m1.predict(Xm_te); test_lgb_reg[te_m] = m2.predict(Xm_te)
...
r1=rmse(y[va1_idx],oof_lgb_bl[va1_idx]); r2=rmse(y[va2_idx],oof_lgb_bl[va2_idx])
...
r1=rmse(y[va1_idx],oof_xgb_pm[va1_idx]); r2=rmse(y[va2_idx],oof_xgb_pm[va2_idx])
print(f"  pm_xgb: 2023={r1:.4f}  2024={r2:.4f}  dual={0.5*(r1+r2):.4f}")

# ─────────────────────────────────────────────────────────────────────────────
# 13. GLOBAL XGB depth=7 — SEED ZOO (7 seeds)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*65)
print(f"GLOBAL XGB depth=7 — {len(D7_SEEDS)} seeds: {D7_SEEDS}")
print("="*65)
...
for seed in D7_SEEDS:
    ...
    m = train_xgb(cfg, X[tr_idx[:-n_es_g]], y_wins[tr_idx[:-n_es_g]],
                  sw[tr_idx[:-n_es_g]], X[tr_idx[-n_es_g:]], y_wins[tr_idx[-n_es_g:]])
    ...
    print(f"  g_xgb_d7_s{seed:<4}: 2023={r1:.4f}  2024={r2:.4f}  iters={m.best_iteration}")
    oof_xgb_d7_list.append(oof); test_xgb_d7_list.append(test_pred)
    del m; gc.collect()

# ─────────────────────────────────────────────────────────────────────────────
# 14. GLOBAL XGB depth=8 — SEED ZOO (5 seeds)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*65)
print(f"GLOBAL XGB depth=8 — {len(D8_SEEDS)} seeds: {D8_SEEDS}")
print("="*65)
...
for seed in D8_SEEDS:
    ...
    test_pred = m.predict(X_test).astype(np.float32)
    r1=rmse(y[va1_idx],oof[va1_idx]); r2=rmse(y[va2_idx],oof[va2_idx])
    print(f"  g_xgb_d8_s{seed:<4}: 2023={r1:.4f}  2024={r2:.4f}  iters={m.best_iteration}")
    oof_xgb_d8_list.append(oof); test_xgb_d8_list.append(test_pred)
    del m; gc.collect()

# ─────────────────────────────────────────────────────────────────────────────
# 15. GLOBAL LGB d127 + d255
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*65 + "\nGLOBAL LGB d127 + d255\n" + "="*65)

n_es_glgb = max(int(len(tr_idx)*0.1), 500)
lgb_d127  = train_lgb(LGB_GLOBAL_D127, X[tr_idx[:-n_es_glgb]], y_wins[tr_idx[:-n_es_glgb]],
                      sw[tr_idx[:-n_es_glgb]], X[tr_idx[-n_es_glgb:]], y_wins[tr_idx[-n_es_glgb:]])
...
lgb_d255  = train_lgb(LGB_GLOBAL_D255, X[tr_idx[:-n_es_glgb]], y_wins[tr_idx[:-n_es_glgb]],
                      sw[tr_idx[:-n_es_glgb]], X[tr_idx[-n_es_glgb:]], y_wins[tr_idx[-n_es_glgb:]])
...
# ─────────────────────────────────────────────────────────────────────────────
# 16. GLOBAL LGB DART — SEED ZOO (4 seeds)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*65)
print(f"GLOBAL LGB DART — {len(DART_SEEDS)} seeds: {DART_SEEDS}")
print("="*65)
...
for seed in DART_SEEDS:
    ...
    oof_dart_list.append(oof); test_dart_list.append(test_pred)
    del m; gc.collect()

# ─────────────────────────────────────────────────────────────────────────────
# 17. SLSQP 20-MODEL WEIGHT OPTIMISATION  (pure 2024, 100 starts/market)
# ─────────────────────────────────────────────────────────────────────────────
N_D7   = len(D7_SEEDS)
N_D8   = len(D8_SEEDS)
N_DART = len(DART_SEEDS)
N_TOTAL = 2 + N_D7 + N_D8 + 2 + N_DART  # pm_lgb + pm_xgb + d7s + d8s + lgb2 + darts

print(f"\n{'='*70}")
print(f"SLSQP {N_TOTAL}-MODEL WEIGHT OPTIMISATION  (pure 2024, 100 starts/market)")
print(f"{'='*70}")

def optimize_weights_slsqp(preds_list, y_true, n_starts=100, seed=42):
    n   = len(preds_list)
...
print(f"  {'Mkt':<10} " + " ".join(f"{n:>7}" for n in model_names) + "     2024    2023")
print("  " + "-"*150)
...
for mkt in MARKETS:
    ...
    preds2 = ([oof_lgb_bl[va2_m], oof_xgb_pm[va2_m]] +
              [o[va2_m] for o in oof_xgb_d7_list] +
              [o[va2_m] for o in oof_xgb_d8_list] +
              [oof_lgb_d127[va2_m], oof_lgb_d255[va2_m]] +
              [o[va2_m] for o in oof_dart_list])
    preds1 = ([oof_lgb_bl[va1_m], oof_xgb_pm[va1_m]] +
              [o[va1_m] for o in oof_xgb_d7_list] +
              [o[va1_m] for o in oof_xgb_d8_list] +
              [oof_lgb_d127[va1_m], oof_lgb_d255[va1_m]] +
              [o[va1_m] for o in oof_dart_list])

    w, r2_opt = optimize_weights_slsqp(preds2, y2)
    best_market_weights[mkt] = w
    P1 = np.column_stack([p.astype(np.float64) for p in preds1])
    r1 = rmse(y1, P1 @ w)
    wfmt = " ".join(f"{wi:7.3f}" for wi in w)
    print(f"  {mkt:<10} {wfmt}   {r2_opt:7.4f} {r1:7.4f}")

oof_pm_ens = np.zeros(len(X), np.float32)
for mkt in MARKETS:
    mc    = f'mkt_{mkt}'
    ...
    oof_pm_ens[va_all] = sum(w[i]*all_oofs[i] for i in range(N_TOTAL))

r1_pm = rmse(y[va1_idx], oof_pm_ens[va1_idx])
r2_pm = rmse(y[va2_idx], oof_pm_ens[va2_idx])
dual_pm = 0.5*(r1_pm + r2_pm)
print(f"\n  Ensemble: 2023={r1_pm:.4f}  2024={r2_pm:.4f}  dual50={dual_pm:.4f}")
print(f"  V33 ref:  2023=30.8554  2024=24.4016  LB=22.488150")
print(f"  V28 ref:  2023=30.7859  2024=24.3913  LB=22.721653")
print(f"  TARGET:   2024<24.40  (beat V33)")

# ─────────────────────────────────────────────────────────────────────────────
# 18. RETRAIN ON ALL DATA
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "="*65 + "\nRETRAIN ON ALL DATA\n" + "="*65)
...
for mkt in MARKETS:
    ...
    Xm_all = X[all_m][:,pm_idx]; Xm_te = X_test[te_m][:,pm_idx]
...
# Retrain LGB d127/d255
lgb_d127_f  = train_lgb(LGB_GLOBAL_D127, X[:-n_es_gf], y_wins[:-n_es_gf], sw[:-n_es_gf], X[-n_es_gf:], y_wins[-n_es_gf:])
lgb_d255_f  = train_lgb(LGB_GLOBAL_D255, X[:-n_es_gf], y_wins[:-n_es_gf], sw[:-n_es_gf], X[-n_es_gf:], y_wins[-n_es_gf:])
...
for seed in DART_SEEDS:
    cfg = {**DART_BASE, 'random_state': seed}
    m = train_lgb_dart(cfg, X, y_wins, sw)
...
# ─────────────────────────────────────────────────────────────────────────────
# 19. BUILD FINAL PREDICTIONS + SAVE
# ─────────────────────────────────────────────────────────────────────────────
...
assert len(sub) == 13098 and sub['target'].notna().all()
sub[['id','target']].to_csv('submission_v34.csv', index=False)

elapsed = (time.time() - t0) / 60
print(f"\n{'='*60}")
print(f"SAVED: submission_v34.csv")
print(f"Dual RMSE: {dual_pm:.4f}  (2023={r1_pm:.4f}, 2024={r2_pm:.4f})")
print(f"Preds: mean={preds.mean():.2f}  std={preds.std():.2f}  neg={100*(preds<0).mean():.1f}%")
print(f"  → Models: {N_D7} XGB-d7 + {N_D8} XGB-d8 + 2 LGB + {N_DART} DART = {N_TOTAL} total")
print(f"Time:  {elapsed:.1f} min")
print(f"V33 LB=22.488150 (3rd) | V28 LB=22.721653 | 1st=22.214397")
print(f"{'='*60}")
